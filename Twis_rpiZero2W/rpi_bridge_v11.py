...
MSG_TELEM = 0xD2   # 11x float32 -> LEN=44
...
def uart_reader(ser: serial.Serial, udp_telem: socket.socket, shared: Shared):
    ...
    while True:
        ...
        # Telemetry: 11 floats -> 44B
        if msg_type == MSG_TELEM and ln == 44:           
            ax, ay, az, gx, gy, gz, temp, dist_cm, roll_deg, mean_roll_deg, mean_dist_cm = struct.unpack("<11f", payload)

            line = (
                f"ax={ax:.3f},ay={ay:.3f},az={az:.3f},"
                f"gx={gx:.3f},gy={gy:.3f},gz={gz:.3f},"
                f"temp={temp:.2f},dist={dist_cm:.2f},"
                f"roll={roll_deg:.2f},mean_roll={mean_roll_deg:.2f},mean_dist={mean_dist_cm:.2f}"
            )
            ok += 1
